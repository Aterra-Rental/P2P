...
from services.wallet_service import (
    WalletError,
    hold_external_payment_in_escrow,
)
...
def generate_payment_qr(room_code):
    ...
    provider_name = get_payment_provider_name()
    conn = get_db()
    cur = conn.cursor()
    ...
    try:
        ...
        # A QR generated by another provider cannot be reused.
        # Expire it before searching for a reusable attempt.
        if current_payment_provider != provider_name:
            cur.execute(
                """
                UPDATE deal_payment_attempt
                SET status = 'Expired'
...
def verify_payment(room_code):
    authenticated_user_id = g.current_user_id
    provider_name = get_payment_provider_name()
    mock_provider = is_mock_payment_provider()
    conn = get_db()
    cur = conn.cursor()
    ...
    try:
        ...
        if current_payment_provider != provider_name:
            return jsonify({
                ...
                "verified": False,
                "message": (
                    "The payment provider changed. "
                    "Generate a new payment QR."
                ),
            }), 409
        ...
        if (
            not mock_provider
            and seconds_since_last_attempt is not None
        ...
        transaction_reference = str(
            transaction.get("hash") or khqr_md5
        ).strip()

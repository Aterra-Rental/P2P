...
// Socket.IO connects to the host root, not the /api path
const SOCKET_URL = API_URL.replace(/\/api\/?$/, "");

const socket = io(SOCKET_URL, { autoConnect: true });

// ---------------------------------------------------------------------
// IMPORTANT: this figures out which user is currently logged in so that
// read-state is scoped PER ACCOUNT instead of shared across the whole
// browser. Adjust this to match however your app actually stores the
// logged-in user (JWT, user object, etc). Common patterns are tried
// below in order; delete whichever don't apply to your app.
// ---------------------------------------------------------------------
function getUserId() {
  try {
    // Pattern 1: a stored user object, e.g. localStorage.setItem("user", JSON.stringify({ id: 31, ... }))
    ...
    // Pattern 2: a JWT stored directly, e.g. localStorage.setItem("token", "...")
    ...
    console.error("Failed to resolve current user id for notifications", err);
  }

  // Fallback: no logged-in user detected, use a shared "guest" bucket
  // so logged-out visitors don't crash, but this should rarely hit
  // once a real user id source is wired in above.
  return "guest";
}
...
export default function NotificationBell() {
  ...
  const [selected, setSelected] = useState(null); // announcement shown in modal
  ...
  return (
    <div className="bell-wrap" ref={dropdownRef}>
      ...
      >
        <Bell size={20} className={unreadCount > 0 ? "bell-icon-active" : "bell-icon"} />
        {unreadCount > 0 && (
                  ...
                  <div className="bell-item-row">
                    ...
                    <div className="bell-item-body">
                      <div className="bell-item-top">
                        ...
                        <span className="bell-item-time">{timeAgo(a.created_at)}</span>
                      </div>
        ...
        <div className="bell-modal-overlay" onClick={() => setSelected(null)}>
          <div className="bell-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bell-modal-header">
              ...
              <span className="bell-modal-date">{fullDate(selected.created_at)}</span>
            </div>
